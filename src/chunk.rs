...
use crate::{chunk_type::ChunkType, ChunkError};
...
impl Chunk {
    pub fn new(chunk_type: ChunkType, data: Vec<u8>) -> Chunk {
        let crc = CRC.checksum(&[&chunk_type.bytes(), &data[..]].concat());
...
impl TryFrom<&[u8]> for Chunk {
    ...
    fn try_from(value: &[u8]) -> Result<Self, Self::Error> {
        ...
        let expected_crc = CRC.checksum(&[&chunk_type.bytes(), &data[..]].concat());
...
impl Display for Chunk {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        let data: String = self.data.iter().map(|b| format!("{b}, ")).collect();
        let data = data.trim().trim_end_matches(',');

        write!(f, "{chunk_type} chunk of length {length}: {data}")
    }
}
