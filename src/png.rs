...
use crate::{chunk::Chunk, ChunkError, PngError};
...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk, crate::Error> {
        let index = self
            .0
            .iter()
            .position(|c| c.chunk_type().bytes() == chunk_type.as_bytes())
            .ok_or(ChunkError::UnknownChunkType)?;
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        let header = Self::STANDARD_HEADER.into_iter();
        let chunks = self.0.iter().flat_map(|c| c.as_bytes());
        header.chain(chunks).collect()
    }
}
...
impl Display for Png {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let header: String = self.header().iter().map(|b| b.to_string()).collect();
